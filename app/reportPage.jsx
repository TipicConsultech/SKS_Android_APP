...
const ReportPage = ({ navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    const formData = {
      ...
      call_type: parseInt(data3.call_type, 10),
      remark: data3.remark,
    ...
    //   Alert.alert(
    //     'Success',
    //     `Report number ${response.reportDetail.id} is Generated Successfully!`,
    //     [{ text: 'OK', 
    //       onPress: () => {
    //         navigation.navigate('finalReport', {
    //           id: response.reportDetail.id, // Pass data as parameter
    //         });
    //      }}]
    //   );
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.table}>
        ...
        <Text style={styles.tableRow}>Finally Status: {data3.customer_suggestion}</Text>
      </View>
      ...
      {!isSigned && (
        <TouchableOpacity style={styles.customButtonsub} onPress={handleSubmitsign}>
          <Text style={styles.buttonTexts}>Sign</Text>
        </TouchableOpacity>
          )}
